...
try:
    ...
    with open("error.log","a") as e:
        e.write(time.strftime("%Y-%m-%d") + "_" + time.strftime("%H:%M:%S") + " \"Paramiko\" module missing! Please visit http://www.paramiko.org/installing.html for more details." + "\n")
    e.close()
...
for line in f:
    ...
    try: 
        ...
        with open("error.log","a") as e:
            e.write(conn_date + "_" + conn_time + " " + mt_username + "@" + host + " Timeout connecting to the device." + "\r\n")
        e.close()
        continue
        ...
        with open("error.log","a") as e:
            e.write(conn_date + "_" + conn_time + " " + mt_username + "@" + host + " Wrong credentials." + "\r\n")
        e.close()
        continue
        ...
        with open("error.log","a") as e:
            e.write(conn_date + "_" + conn_time + " "  + mt_username + "@" + host + " Unknown error while connecting to the device." + "\r\n")
        e.close()
        continue
    ...
    with open("success.log","a") as s:
        s.write(conn_date + "_" + conn_time + " Successfully executed commands on the host " + host + "\n")
    s.close()
    k.close()
